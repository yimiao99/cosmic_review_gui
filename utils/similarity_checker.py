...
class SimilarityChecker:
    ...
    REQUIRED_WORD_MODULES = [
        "1. 需求说明", "1.1. 总体描述", "1.2. 建设目标", "1.3. 建设必要性",
        "2. 功能架构图", "3. 功能需求", 
        "3.1. 功能需求1(请注明本需求是：新增、优化）", 
        "3.1.1. 关键时序图/业务逻辑图", "3.1.2. 功能描述", 
        "3.4. 需求功能清单", "4. 附加值调整因子说明", 
        "5.1. 需求变更规模因子", "5.2. 应用领域", "5.3. 质量及特性", 
        "5.4. 开发语言", "5.5. 开发团队背景", "5.6. 完整性级别调整因子"
    ]
    ...
    def validate_template(cls, target_sections, excel_info, template_sections=None):
        ...
        for s in target_sections:
            target_lookup[clean_text(s['title'])] = s

        # 2. 索引模板内容 (作为参考库)
        template_content_map = {}
        if template_sections:
            for s in template_sections:
                template_content_map[clean_text(s['title'])] = s.get('content', "")
        ...
        for m in cls.REQUIRED_WORD_MODULES:
            ...
            if target:
                ...
                if level == 1: results["is_valid"] = False

            # 获取模板预设值
            base_content = template_content_map.get(m_clean, "").strip()
            
            # --- 逐句拆分对比 ---
            base_sents = [s.strip() for s in re.split(r'[。\n!！?？;；]', base_content) if len(s.strip()) > 3]
            target_sents = [s.strip() for s in re.split(r'[。\n!！?？;；]', target_content) if len(s.strip()) > 3]
            
            # 情形 A: 章节缺失
            if not target:
                results["all_details"].append({
                    "level": level, "chapter": m, "status": "缺失",
                    "tpl_sentence": base_content[:50] + "..." if base_content else "模板无预设正文",
                    "target_sentence": "-", "score": 0.0
                })
                continue
            ...
            for ts in target_sents:
                ...
                if best_sim > 0.4:
                    ...
                    if best_sim > 0.85: section_suspect_count += 1
                
                comparison_rows.append({
                    "level": level, "chapter": m, "status": status,
                    "tpl_sentence": tpl_text, "target_sentence": ts, "score": best_sim
                })
            ...
            for idx, bs in enumerate(base_sents):
                if idx not in marked_tpl_idx:
                    comparison_rows.append({
                        "level": level, "chapter": m, "status": "🔹 模板参考",
                        "tpl_sentence": bs, "target_sentence": "(未提及)", "score": 0.0
                    })
            
            # 如果章节本身没内容
            if not target_sents and not base_sents:
                results["all_details"].append({
                    "level": level, "chapter": m, "status": "已匹配",
                    "tpl_sentence": "(空)", "target_sentence": "(空)", "score": 1.0
                })
            else:
        ...
        extra_chapters = []
        for s in target_sections:
            s_clean = clean_text(s['title'])
            if s_clean not in seen_mandatory_cleans:
        ...
        results["template_match_rate"] = found_mandatory / total_mandatory if total_mandatory > 0 else 0
